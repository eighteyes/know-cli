...
if [[ "${AI_MODE:-false}" == "true" ]]; then
    TEMPLATE_FILE="$SCRIPT_DIR/../templates/feature-ai.md"
else
    TEMPLATE_FILE="$SCRIPT_DIR/../templates/feature.md"
fi
...
generate_feature_spec() {
    ...
    if [[ ! "$entity_ref" =~ ^features: ]]; then
        error "Expected features entity, got: $entity_ref"
    fi
