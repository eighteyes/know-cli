...
function initializeEventListeners() {
    ...
    // Sidebar Modifications Panel
    document.getElementById('sidebar-submit-changes').addEventListener('click', handleSidebarSubmitChanges);
    document.getElementById('sidebar-clear-changes').addEventListener('click', clearSidebarModifications);

    // Use event delegation for dynamically created checkboxes
    document.getElementById('sidebar-modifications').addEventListener('change', (e) => {
        if (e.target.type === 'checkbox') {
            updateSidebarSubmitButton();
        }
    });
...
function updateDiscoverProgressBar() {
    ...
    if (!progressBar && discoverBtn) {
        ...
        // Insert inside the discover button
        discoverBtn.appendChild(progressBar);
    }
...
function navigateToPage(pageName) {
    ...
    if (pageName === 'start') {
        ...
        hideSidebar();  // Hide sidebar when going to start page
    } else {
        topBar.classList.remove('hidden');
        showSidebar();  // Show sidebar for all other pages
...
async function extractEntitiesReferences(text, isBatchExtraction = false) {
    try {
        ...
        // Use the new sidebar modifications panel instead of the old extraction UI
        populateSidebarModifications(data);


        // Keep the old extraction UI hidden now that we're using the sidebar
        document.getElementById('extraction-choices').classList.add('hidden');
...
// Show sidebar when transitioning from start page
...
// Hide sidebar when returning to start page
...
// Sidebar Modifications Panel Functions

// Populate sidebar modifications panel with entities and connections
function populateSidebarModifications(data) {
    const entitiesList = document.getElementById('sidebar-entities-list');
    ...
    // Add entities with checkboxes
    if (data.entities && data.entities.length > 0) {
        data.entities.forEach(entity => {
            const li = document.createElement('li');
            li.innerHTML = `
                <input type="checkbox" id="entity-${entity.type}-${entity.name}" data-type="entity" data-entity-type="${entity.type}" data-name="${entity.name}" data-reasoning="${entity.reasoning || ''}">
                <label for="entity-${entity.type}-${entity.name}">
                    ${entity.name}
    ...
    // Add connections with checkboxes
    if (data.connections && data.connections.length > 0) {
        data.connections.forEach(conn => {
            const li = document.createElement('li');
            li.innerHTML = `
                <input type="checkbox" id="conn-${conn.from}-${conn.to}" data-type="connection" data-from="${conn.from}" data-to="${conn.to}" data-reasoning="${conn.reasoning || ''}">
                <label for="conn-${conn.from}-${conn.to}">
                    ${conn.from} → ${conn.to}
                </label>
            `;
            connectionsList.appendChild(li);
        ...
        connectionsList.innerHTML = '<li>No connections detected</li>';
    }

    // Show the sidebar modifications panel
    document.getElementById('sidebar-modifications').classList.remove('hidden');
...
function updateSidebarSubmitButton() {
    const submitBtn = document.getElementById('sidebar-submit-changes');
    const checkedBoxes = document.querySelectorAll('#sidebar-modifications input[type="checkbox"]:checked');

    submitBtn.disabled = checkedBoxes.length === 0;
    submitBtn.textContent = checkedBoxes.length === 0 ? 'Add Selected' : `Add Selected (${checkedBoxes.length})`;
}

// Handle sidebar submit changes
function handleSidebarSubmitChanges() {
    const checkedBoxes = document.querySelectorAll('#sidebar-modifications input[type="checkbox"]:checked');

    checkedBoxes.forEach(checkbox => {
        if (checkbox.dataset.type === 'entity') {
            addEntity(checkbox.dataset.entityType, checkbox.dataset.name, checkbox.dataset.reasoning);
        } else if (checkbox.dataset.type === 'connection') {
            addConnection(checkbox.dataset.from, checkbox.dataset.to);
        }
    });

    // Clear sidebar modifications after adding
    clearSidebarModifications();

    // Hide the old extraction choices UI if it's visible
    document.getElementById('extraction-choices').classList.add('hidden');
...
// Clear sidebar modifications panel
function clearSidebarModifications() {
    document.getElementById('sidebar-entities-list').innerHTML = '';
    document.getElementById('sidebar-connections-list').innerHTML = '';
    document.getElementById('sidebar-modifications').classList.add('hidden');

    // Uncheck all checkboxes
    document.querySelectorAll('#sidebar-modifications input[type="checkbox"]').forEach(cb => {
        cb.checked = false;
    });
...
// Toggle modifications panel collapse
function toggleModPanel() {
    const content = document.querySelector('.modifications-content');
...
window.toggleModPanel = toggleModPanel;
window.handleSidebarSubmitChanges = handleSidebarSubmitChanges;
window.clearSidebarModifications = clearSidebarModifications;